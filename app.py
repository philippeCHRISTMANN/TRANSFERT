...
st.info("💡 Prenez les tableaux d'amortissement de votre financement actuel (Prêt à Taux Zéro + Prêt Principal). Nous allons reconstituer votre effort financier actuel.")
...
with col_ptz:
    ...
    if is_constant_ptz.startswith("Oui"):
        mens_ptz = st.number_input("Mensualité hors assurance PTZ (€)", min_value=0.0, step=10.0, value=150.0)
        duree_ptz = st.number_input("Durée totale du PTZ (en mois)", min_value=1, step=12, value=300)
        ptz_flow_total = [mens_ptz] * int(duree_ptz)
    else:
        nb_paliers_ptz = st.number_input("Nombre de paliers du PTZ", min_value=2, max_value=6, value=2, key="nb_pal_ptz")
        for i in range(int(nb_paliers_ptz)):
            c1, c2 = st.columns(2)
            with c1:
                dur_p = st.number_input(f"Durée Palier {i+1} (mois)", min_value=1, step=12, value=120, key=f"ptz_dur_{i}")
            with c2:
...
with col_pp:
    ...
    is_constant_pp = st.radio(
        "Les mensualités du Prêt Principal sont-elles constantes ?",
        ("Oui, toujours la même", "Non, il y a des paliers"),
        key="radio_pp"
    )
    ...
    if is_constant_pp.startswith("Oui"):
        ...
        duree_pp = st.number_input("Durée totale du Prêt Principal (en mois)", min_value=1, step=12, value=300)
        pp_flow_total = [mens_pp] * int(duree_pp)
    else:
        nb_paliers_pp = st.number_input("Nombre de paliers du Prêt Principal", min_value=2, max_value=6, value=2, key="nb_pal_pp")
        for i in range(int(nb_paliers_pp)):
            c1, c2 = st.columns(2)
            with c1:
                dur_p = st.number_input(f"Durée Palier {i+1} (mois)", min_value=1, step=12, value=120, key=f"pp_dur_{i}")
            with c2:
